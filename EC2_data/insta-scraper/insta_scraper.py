...
def get_scraped_data():
    ...
    profilepic_url = soup.find('meta', {'property': 'og:image'
                               })['content']
    db.child('instagram').child('url').set(profilepic_url,
            user['idToken'])
...
class InstagramScraper(object):
    ...
    def save_json(data, dst='./'):
        ...
        if data:
            ...
            check(json.dumps(data), stored_json)
...
def main():
    args = dict(
        ...
        location=False,
        login_only=True,
        login_pass='YOUR INSTA PASSWORD',
        login_user='YOUR INSTA USERNAME',
        maximum=20,
        media_metadata=True,
        media_types=['none'],
        quiet=False,
...
def check(json_list_1, stored_json):
	json_list=json.loads(json_list_1)
	print('stored id = ', stored_json['data'][0]['id'], "scraped id = ",json_list[0]['id'])
	if(json_list[0]['id'] != stored_json['data'][0]['id']):
		print('gonna insert data')
		db=firebase.database()
		db.child("instagram").child("data").set(json_list,user['idToken'])
